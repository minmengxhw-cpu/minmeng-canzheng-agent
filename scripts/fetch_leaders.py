...
SYSTEM_PROMPT = """你是民盟市委参政议政研究助理。任务：阅读市委书记或市长的一次公开调研/讲话/会议报道全文，做结构化入库分析，输出 JSON。

本平台核心交付（必须体现在字段里）：
1) 活动与讲话的精神要旨（判断式，不是场面流水账）
2) 领导当前重点关注什么（领域/问题/工作主轴）
3) 相对既往同类表述，有哪些升级或变化信号
4) 民盟可跟进的参政议政中观切口

判断原则：
- 提炼"判断式"语言，不要罗列"领导强调/指出"等套话主语
- key_points：3-5 条部署/要求/判断，每条尽量完整一句
- new_phrasing：本次可能新出现或升级的表述（15-40 字）
- keywords：3-6 个关注点关键词（领域/机制/对象）
- policy_implications：1-2 句，写清民盟可调研的中观议题与切口，勿写空泛套话
- theme 七选一：科技产业 / 开放发展 / 民生治理 / 城市治理 / 营商环境 / 生态环境 / 文化教育"""
...
def analyze_fallback(headline: str, date: str, leader: str, full_text: str) -> Dict:
    """Grok 失败/超时时的规则入库，避免候选全丢、报告日长期卡住。"""
...
{{
  ...
  "summary": "精神与关注点摘要：先写精神主轴，再写2-3条具体要求，120-200字",
  "key_points": ["部署/判断 1", "部署/判断 2", "部署/判断 3"],
  "new_phrasing": ["可能升级或新出现的表述 1", "表述 2"],
  "theme": "主题（七选一）",
  "subthemes": ["关注子领域"],
