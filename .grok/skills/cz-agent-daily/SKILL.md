...
description: >
  ...
  负责：每日 08:30 仅推一次外部飞书群、MiniMax 分析流水线、政务体简报格式、
...
**目标：** 每天 **只** 在 **08:30** 向 **指定外部飞书群** 推送 **一份** 政务体简报，由 **MiniMax** 做分析，**绝不发错群、不双推、不发晚报**。
...
4. **执行引擎**  
   - 分析：`mmx` / MiniMax-M3（`MINIMAX_CLI`、`MINIMAX_MODEL`）  
   - 简报拼装：`gen_brief.py` 规则政务体（无省略号）  
   - 额度不足：中央通道可规则兜底，但流水线仍须跑完并尽量推飞书
...
launchd 08:30
  → update_and_push.sh
      ...
      → update_all.py
            → fetch_central.py   # MiniMax / 兜底
            → fetch_leaders.py   # MiniMax
...
- 结构：一、要情导读 → 二、中央层面 → 三、上海层面 → 四、近七日上海主题观察 → 编校说明  
- 各层面：（一）关键词（二）重要表述（三/四）动态摘要  
- **禁止**正文用 `…` / `...` 截断；超长减条数不裁句  
- 中央含：最高层考察、政治局会议、**党外人士座谈会**、总理考察  
...
分析可能失败/走兜底；仍应生成简报并推送。提示用户 `mmx quota show`。
